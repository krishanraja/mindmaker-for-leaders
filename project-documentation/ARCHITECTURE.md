...
**Last reconciled:** 2026-07-12 (drift reconciliation pass).

> **Drift reconciliation pass (2026-07-12)**: verified counts against the code today: **104 edge functions** (`supabase/functions/`, excluding `_shared/`), **78 hooks** (`src/hooks/`), **148 migrations** (`supabase/migrations/`). The "UX Redesign (2026-06-17)" section below (Cockpit Home / `CockpitDeck` / `VITE_COCKPIT_ENABLED`) and the Memory Web `MobileMemoryDashboard`/`DesktopMemoryDashboard` references are now **HISTORICAL AND DELETED** - superseded first by the unified onboarding->decisions->engagement loop (2026-06-29), then by the CTRL 2028 radical-focus refactor (PRs #234-241) which replaced them with `HomeFeed.tsx` / `DesktopHomeView.tsx` (`src/components/cockpit/`). The primary nav is no longer 4 tabs; it is now **3 tabs (Home / Decisions / Memory)** - `src/components/layout/DesktopShell.tsx` `primaryNavItems` and `src/components/memory-web/BottomNav.tsx` `navItems` both confirm Track Record folded into the Decisions tab (Now|History toggle) and Edge/Export/Goals/Briefing/Decision Map demoted to secondary/account nav (still routed, just not primary). The routes table was missing `/agents`, `/try`, `/download`, `/upgrade`, `/track-record`, `/decision-map`, and `/kit/pdf[/:redemptionId]` - added below from `src/router.tsx`. The Deployment section's "`supabase db push`" instruction is corrected: per `CLAUDE.md` the local migration history is out of sync with remote, so migrations are applied via the Supabase Management API, never `supabase db push`. See `CLAUDE.md` at repo root for the fuller current narrative (posture/userState, curation system, kit redesign, settings audit) - it is the more current source for anything shipped after 2026-06-21.
>
...
>
> **Verified counts (re-counted 2026-07-12)**: 104 edge functions, 78 hooks, 148 migrations, 8 e2e specs, 29 non-e2e vitest specs matching the vitest `include` pattern (`src/**` + `supabase/functions/_shared/**` + `supabase/functions/decision-engine/**`), pgvector + pgcrypto + pg_cron extensions enabled, 6 audit-week tracks shipped (revenue path, data path, UX, reliability, observability, cleanup), Phase 8 shipped (Skill Builder + desktop UI redesign + pain-anchored entry points), Phase 9 shipped (Decision Engine + flag-gated Briefing streaming + cross-tenant RLS hardening), Phase 10 shipped (every authenticated surface unified onto `DesktopShell`, viewport-pinned zero-scroll, Goals + Enrich loop). These counts will drift again as work lands; treat `CLAUDE.md`'s own count callouts (which carry their own dated caveats) and a fresh `ls`/`wc -l` as the tiebreaker over any hardcoded number here.
>
...
## UX Redesign (2026-06-17, PRs #197-200) - HISTORICAL, DELETED

> **This section is preserved for history only; none of the components it names still exist in the code as of 2026-07-12.** `CockpitHome.tsx` and `CockpitDeck.tsx` were DELETED by the CTRL 2028 refactor (PR #237) and replaced by `src/components/cockpit/HomeFeed.tsx` (mobile swipe-feed) + `src/components/cockpit/DesktopHomeView.tsx` (desktop rail), unified with the old Memory Web dashboards - `MobileMemoryDashboard.tsx` / `DesktopMemoryDashboard.tsx` were ALSO deleted, not just the cockpit ones. The `VITE_COCKPIT_ENABLED` flag that gated this section is RETIRED (the cockpit model is the only Home). See `CLAUDE.md`'s "CTRL 2028 radical-focus refactor" and "Unified onboarding -> decisions -> engagement loop" entries for the current model, and `docs/CTRL-SYSTEM-SPEC.md` section 6 for the design language that replaced it.
