...
> **Current CTRL overlay, 2026-08-10:** user overwhelm is poison. CTRL is one product at `makeyourmindup.ai`; Make Your Mind Up is the warm intake. The primary surfaces are Today, Decide, Blind Spot, Memory, Settings, and the conversational audio briefing. One useful ask at a time, 44px signature targets, no clipped meaning, no duplicate feeds or tools, and no setup work that exists only to feed the app. Preserve premium category visualization. Use Segoe UI Variable Display/Text as one optical family and mono only for metadata. Blind Spot replaces Skill Builder as the promoted leadership-development surface. The value must continue through no-login email/audio delivery. Production claims require live readback, not “should be deployed.” This overlay supersedes conflicting phase-era product guidance below.
