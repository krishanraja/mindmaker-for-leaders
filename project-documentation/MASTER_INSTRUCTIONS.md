...
> **RECONCILIATION DISPOSITION (2026-08-02): MERGE/DELETE - recommended for removal, not deleted here.** This document is a generic AI-assistant-behavior template for "vibe-coded" projects built in Lovable (see section 0's "work for any codebase in Lovable" and section 8's "HOW TO PROMPT / ITERATE INSIDE LOVABLE"). It contains ZERO CTRL-specific content: no mention of CTRL, Mindmaker, Supabase project `bkyuxvschuwngtcdhsyg`, the edge-function inventory, or any fact about this codebase. This repo is developed with Claude Code + the Supabase CLI + `gh`, not Lovable, and the root `CLAUDE.md` is the document that actually governs assistant behavior here now - and in at least one place directly CONTRADICTS this file's generic advice: section 3.5 here says "Use proper migrations" with no caveat, where `CLAUDE.md` documents that `supabase db push` is broken for this repo (local migration history out of sync with remote) and that migrations must go through the Supabase Management API instead. Following this document's generic guidance over `CLAUDE.md`'s repo-specific guidance would break the migration workflow. TODO(founder): confirm removal; if kept, it should be re-scoped as explicitly Lovable-specific boilerplate (not "permanent project knowledge" for CTRL) or superseded outright by `CLAUDE.md`. Per the reconciliation pass's instructions, this file's own date/content is left untouched pending that decision.
