...
**Last reconciled:** 2026-08-09 (drift-check pass: the Kit program (PART 3, PART 5, PART 6 FAQ) was RETIRED 2026-08-07, PR #355 - `/kit*` now redirects to `/try`; the Kit-entry FAQ at the bottom of PART 6 described a flow that no longer exists and has been corrected. Repo counts in PART 3 were badly stale (Phase-10-era snapshot) and are now current. `public/.well-known/product.json` (ratified 2026-08-04) also resolves the diagnostic-SKU and Mindmaker-Sprint-for-CTRL question below from TODO to dead - see PART 5.)

> RECONCILIATION BANNER. The Mindmaker founder narrative (PARTS 1-2, 4), the enterprise-buyer pain, and the Builder Economy theme are all still accurate and largely ON-positioning (the "AI-native competitors move faster by default" and "everyone will build" themes align with the current CTRL positioning) - PART 4 describes Mindmaker Sprints, a themindmaker.ai offering, not a CTRL SKU; keep that distinction sharp, especially since PART 5's pricing table used to blur it. The one correction to PART 3: the CTRL product description was written as "a portable AI memory web / context layer". CTRL's positioning is now **building the AI-native version of your business** (`product.json`'s shorter public tagline: "Your AI-native chief of staff"; the memory web is the substrate, not the headline). The pricing table (PART 5) is accurate: Edge Pro $49/month (the decision tier), the daily briefing and Automator builds free; the one-time diagnostic SKUs and the Mindmaker Sprint rows are DEAD for CTRL, not TODO - `product.json`'s guardrails say so explicitly. Canonical: `AGENT_BRIEFING.md` + `docs/MAIN-APP-POLISH-SPEC.md` + `product.json`.
...
**What is CTRL?**
The tool for building, orchestrating, productizing, and getting to market the AI-native version of your business. It reframes every general-business call into its AI-native version and pulls you there. Underneath it runs a private memory web (the substrate that makes any AI tool know your business), a news deck of AI-native categories, a decision engine that reframes and pressure-tests, and an Automator that turns a weekly workflow into an installable agent skill. (The "four guided lesson kits" that used to be part of this description were RETIRED 2026-08-07, PR #355.)
...
- Supabase backend: PostgreSQL with pgvector + pgcrypto + pg_cron, **105 edge functions** (Deno, incl. `generate-skill-export` and the `decision-engine` trio; re-counted 2026-08-09), **79 React hooks**, **154 migrations** applied
...
| SKU | Price | Audience |
|---|---|---|
| CTRL Free | $0 | Every leader. The land in land-and-expand. Includes read-write Memory Web, Voice Profile capture, and **Automator skill builds + exports (free for now, no quota)**. |
...
Full plan matrix: see `docs/PRICING.md`, `product.json`, and the `PlanMatrix` block in the Edge Pro settings tab. CTRL is two tiers only: per `product.json`'s guardrails (ratified 2026-08-04), the one-time Diagnostic ($49) / Deep Context ($29) / Bundle ($69) SKUs are dead, and the Mindmaker Enterprise Sprint ($15K-$50K) / Portfolio Engagement ($5K-$25K) rows that used to appear here are Mindmaker advisory offerings (retired for CTRL entirely in July 2026; any revived advisory engagement lives on themindmaker.ai, not inside CTRL) - do not quote either for CTRL. The Kit-side-door entry path (anonymous students from `/kit`) that used to graduate into the Free tier was RETIRED 2026-08-07, PR #355.
...
**Q: How do new leaders enter for free without a credit card?**
A: Free signup at `/auth` requires no payment method - Free is a real daily instrument, not a trial. **RETIRED 2026-08-07 (PR #355):** the Kit program's class-QR anonymous-session side door (`/kit` → `upgradeAnonymousSession()`) that used to answer this question no longer exists; `/kit*` now redirects to `/try`, a pre-login pressure-test demo that still requires signup to save a Memory Web.
