...
> **RECONCILIATION BANNER (last reconciled 2026-08-02, first written 2026-06-21).** This Corpus was written under the earlier "clarity engine" positioning. That headline is now SUPERSEDED. The LOCKED positioning (2026-06-19) is: CTRL is the tool for building, orchestrating, productizing, and getting to market **the AI-native version of your business**, not a general clarity engine and not a general business advisor. General-business inputs are reframed into the AI-native lens. Read this Corpus for its product philosophy and the intelligence-layer thinking (still valuable), but where it frames CTRL as "clarity / see one step clearer", substitute the AI-native positioning. Canonical: `docs/MAIN-APP-POLISH-SPEC.md` (esp. section 0), `docs/KIT-REDESIGN-SPEC.md`, root `README.md`. TODO(founder): decide whether this Corpus is rewritten to the AI-native positioning or retired in favour of the two specs.
>
> **Also now historical (shipped since the 2026-06-21 pass, verified against code/git log 2026-08-02).** Two of this Corpus's own "honest residual gaps" (§14/§15) are CLOSED: the brain canvas Strengthen/Fix actions, listed throughout Part II as UI-disabled pending a backend RPC, went LIVE in PR #321 (evidence-corpus sharpening, 2026-07-03) - corrections now log real `user_corrected`/`user_rejected`/`user_disputed` events the extractor respects. That same PR also added a board-ready decision memo on every weigh and a one-click context-file export, both squarely inside this Corpus's Part II "legible output" thesis. Also shipped since: the unified onboarding -> decisions -> engagement loop (PR #298, 2026-06-29, deleted the `VITE_COCKPIT_ENABLED` fork and legacy Memory dashboards); the Decisions tab rebuilt as a radial force spider (PRs #308-320); a Settings/design-system audit (2026-07-04); Edge Pro repriced to $49/month and repositioned as the decision tier, with the daily briefing now free (PRs #326-327); and a founder-signed North Star flywheel metric - "context in, judgment out, recurring" (PR #330, 2026-07-04, `project-documentation/NORTH_STAR.md`) - which is the closest thing yet to an instrumented answer to this Corpus's §11 moat claim. None of this changes the Corpus's philosophy; it further closes gaps this document already disclosed honestly. Canonical current sources: `docs/CTRL-SYSTEM-SPEC.md` (sections 6-9), `project-documentation/NORTH_STAR.md`, `project-documentation/DECISIONS_LOG.md` (Decisions 59-63), `CHANGELOG.md`.
...
- **Brain edges are derived, not stored** - the fact-to-fact graph is computed rather than persisted. (The brain canvas Strengthen/Fix actions, previously listed here as UI-disabled, went LIVE in PR #321, 2026-07-03; removed from this list.)
