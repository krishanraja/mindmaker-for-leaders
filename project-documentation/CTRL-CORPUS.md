...
>
> **Re-verified 2026-07-12.** Positioning banner above still holds; no further rewrite. One factual update against the live code: PR #321 (2026-07-03, "evidence-corpus sharpening") shipped the backend RPCs behind the brain canvas Strengthen/Fix actions and wired a caller for edge derivation - several "residual gap" notes below describing those as disabled/unwired are now stale. Corrected inline where they occur; see `docs/CTRL-SYSTEM-SPEC.md` section 9 for the canonical current state.
...
- **Self-Correction** *(SHIPPED - the keystone is built).* The loop that turns every correction into a kept rule, so the same mistake doesn't recur. It was the deck's centrepiece and the thing the app most conspicuously lacked; it now exists, governed by Law 1 - built on corrections the leader actually makes in the flow of getting value, not on a fantasy of constant reporting. (Update 2026-07-12: the brain canvas Strengthen/Fix *actions* were UI-present-but-disabled as of this Corpus's last edit; PR #321 (2026-07-03) shipped the backend RPCs and they are now LIVE. See `docs/CTRL-SYSTEM-SPEC.md` section 9.)
...
- **Store.** `user_memory` is a temperature-tiered fact graph (category, confidence, verification status, `temperature ∈ hot|warm|cold`, `reference_count`, version pointers, encrypted content). Around it: pattern, decision, and budget satellites; the seven Decide tables; the training material. The brain engine added a fact-to-fact **edge graph** (`20260616120000_memory_edges`) and evidence tiers on top (#187-189). *Update 2026-07-12: as of PR #321, `memory-edges-derive` is called after every successful capture (`useMemoryWeb.submitInput`) and writes rows into `memory_edges`, so edges are now persisted, not only computed on read - see the corrected residual-gaps note below.*
