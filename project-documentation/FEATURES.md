...
> **CTRL 2028 radical-focus refactor (2026-06-22, PRs #234-241; LIVE, further superseding the cockpit-deck description below)**: Home was rebuilt again onto `HomeFeed.tsx` / `DesktopHomeView.tsx`; `CockpitHome` / `CockpitStreamRow` / `CockpitDeck` (described in the "Home / Decision Map / Automator UX Redesign" section further down) were DELETED. The desktop shell's primary nav is now 3 stable tabs - **Home / Decisions / Memory** (`src/components/layout/DesktopShell.tsx`, matching the mobile `BottomNav`) - with everything else (Edge, Briefing, Track record, Export, Goals, Decision Map) collapsed behind a "More" disclosure (default closed, auto-opens on a secondary route); Settings/Compliance/Profile sit under a separate Account group. PR #329 (2026-07-04) fixed the one open nit from this refactor: the desktop sidebar footer now shows the email only when a real display name resolved, so a name-less account shows a clean "You" instead of a raw email/id. Canonical: `CLAUDE.md` and `docs/CTRL-SYSTEM-SPEC.md` section 6. The Dashboard/Navigation prose elsewhere in this doc (4 tabs: Home, Edge, Memory Web, Export) is HISTORICAL.

> **Home card unification (2026-07-07, PR #334; LIVE)**: the Home feed's news card (`CockpitHero`) and the structural-shift/trend card (`TrendCard`, PRs #331-332, a distinct Home card type for AI-native structural shifts) now share ONE tap target with a single "Read" affordance (dropped the earlier divergent "More" / "See what's shifting" CTAs); visually distinct (news = neutral graphite surface, shift = emerald-tinted surface with a soft edge glow). The decision CTA was renamed **"Think it through"** (from "Weigh it") across the read sheet, the Home `KickstartCard`, and the decision-capture input.

> **North Star metric (2026-07-04, founder-signed)**: CTRL's moat metric is the **flywheel** - leaders who both hold a real brain (>=5 current `user_memory` facts) and weighed a decision in the last 7 days. Instrumented by migration `20260704120000_north_star_flywheel.sql` (a view, a daily-snapshot table, `snapshot_north_star()`, and a daily pg_cron job). Canonical: `project-documentation/NORTH_STAR.md`.

> **Decision Engine audit fixes (2026-07-04, PR #327)**: (a) the AI-native reframe classifier no longer mistakes the bare word "agent(s)" for an AI signal - a human-agent-phrase guard masks phrases like "support agents" / "sales agents" before the AI-native test, so "hire two more support agents" still gets the reframe it needs, while genuine AI-agent phrases ("build an AI agent") still pass; (b) `_shared/sanitize.ts` strips em/en dashes from every model-generated user-facing decision-engine string (recommendation, counter_case, validate_next, the reframed statement) - the no-em-dash house rule was previously enforced on source code only, not LLM output; (c) a CI vitest job now wires the decision-engine eval gate into CI.

> **Public surface additions (2026-07-04 through 2026-07-07, PRs #327/#331/#333)**: the interactive React pricing page (`src/pages/Pricing.tsx`, live Stripe subscribe button) moved from `/pricing` to **`/upgrade`** (PR #331) - `/pricing` is no longer a React route; `vercel.json` now rewrites it to a static `public/pricing.html` SEO page with no live subscribe button. Edge Pro is still **$49/month** (`EDGE_PRO_UNIT_AMOUNT_CENTS` in `_shared/edge-pricing.ts`, unchanged; only the surface moved). A new feature-flagged (`FF.publicCapture`) public route **`/download`** (`src/pages/CaptureLanding.tsx`, PR #333) is a one-screen no-scroll email-capture landing page backed by the new public `capture-lead` edge function; flag off shows the standard not-found page.
