...
> Reconciliation note (last verified 2026-08-02, previously 2026-06-21): the token-contract structure below is still correct, spot-checked directly against `src/index.css` and `src/styles/tokens.css`. CTRL's own live SKIN is now the globally dark `ctrl-ds` instrument palette (emerald `#00D9B6`), not a light/"mint" skin; any "mint palette" reference below means CTRL's accent, which is now emerald. Verified 2026-08-02: `index.css` defines the full required semantic set (background/card/popover/foreground/muted-foreground/accent/ring/primary/secondary/success/destructive/border/input/muted/radius) on a single `:root, .dark` block (dark forced, identical values both ways, matching section 1's ownership rule); `--success` is defined and wired into `tailwind.config.ts` (the gap section 1 called out is fixed); `tokens.css` still owns the brand primitives (`--mint: 171 100% 43%` = the same emerald) plus the spacing/radius/shadow scales described in section 2; the em-dash + token build-time guard section 5 describes is live in `.github/workflows/ci.yml`. The live tokens are in the code (`src/styles/tokens.css`, `src/index.css`).
