...
**Last reconciled:** 2026-07-19 (routine reconciliation pass). Spot-checked against the live code: `--success` is still defined in both `src/index.css` and referenced in `tailwind.config.ts`; `_shared/with-timeout.ts` still exports `fetchWithTimeout` + `ProviderUnavailableError` as described below. No drift found.
