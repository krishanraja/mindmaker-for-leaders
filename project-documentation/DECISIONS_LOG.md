...
**Last reconciled:** 2026-07-12 (drift-check pass; Decision 61 appended below for PRs #326-334, which had not been recorded here).
...
## Decision 61: Post-Launch Punch List - Deploy Unblock, Money-Path Repair, Growth Surfaces (PRs #326-334, 2026-07-04 to 2026-07-07)
**Date**: 2026-07-04 to 2026-07-07
**Decision**: Ship nine small, mostly sequential PRs closing out the immediate punch list after the $49 Edge Pro reprice (Decision 60): (1) #326 unblocked the prod deploy pipeline, removed em dashes from `CHANGELOG`, added a `standards` CI job. (2) #327 repaired the Edge Pro subscription money path and finished the $49/month reprice + decision-tier repositioning in code. (3) #328 made the decision-engine reframe surface consistently, sanitized its output, and added a vitest job wiring a decision-engine eval gate (AI-native classifier, human-agent reframe guard, no-em-dash sanitizer) into CI. (4) #329 cleaned the sidebar footer, added a collapsible desktop nav, shipped the static `/pricing` SEO page, and added a decision-peak upgrade CTA. (5) #330 instrumented the North Star flywheel metric (context in -> judgment out -> recurring usage). (6) #331 split pricing into two routes: `/pricing` (static SEO, corrected to $49 + the decision-tier positioning) and `/upgrade` (the interactive checkout page). (7) #332 added a Home "shift" card surfacing structural news trends. (8) #333 added a feature-flagged public email-capture page at `/download`. (9) #334 fixed Home card/loading and Decisions-tab UX issues.
**Rationale**: The $49 reprice (Decision 60) needed its money path and public pricing surfaces actually repaired and made consistent (Stripe, code, `/pricing`, `/upgrade`, docs) rather than left as a documentation-only decision; the decision engine needed a first automated eval gate; and the growth surfaces (`/pricing` split, `/download`, the North Star metric, the Home shift card) were queued follow-on work once the pricing/deploy foundation was solid.
**Trade-off**: Nine PRs in four days is a fast cadence; batched here as one decisions-log entry (matching the Decision 42 pattern for coordinated releases) rather than nine, since they share one punch-list intent and none reverses another.
**Outcome**: ✅ All nine merged to main, 2026-07-04 through 2026-07-07. `/pricing` and `/upgrade` are now distinct routes; `/download` and the Home shift card are new surfaces; the decision engine has its first CI eval gate.
