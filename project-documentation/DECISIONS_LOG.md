...
## Decision 61: Founder-Signed North Star Metric - the Flywheel (context in, judgment out, recurring) (PR #330, 2026-07-04)
**Date**: 2026-07-04
**Decision**: Instrument a single founder-signed North Star metric, the **flywheel**: leaders who BOTH (a) hold a real brain (at least 5 current facts in `user_memory`) AND (b) weighed at least one decision in the last 7 days. Neither half alone counts as the product working - a brain that never drives a decision is a filing cabinet; a decision with no brain behind it is a generic answer. Shipped as `north_star_flywheel` (a live view) + `north_star_daily` (a daily snapshot table, RLS on) + `snapshot_north_star()` (idempotent upsert) + a `north-star-daily-snapshot` pg_cron job (06:00 UTC), migration `20260704120000_north_star_flywheel.sql`. Two component counts (`brain_rich_users`, `active_deciders`) plus the engagement denominator (`weekly_active_users`) are tracked alongside so a dip in the flywheel is diagnosable. Documented in `project-documentation/NORTH_STAR.md`.
**Rationale**: The repo had no written North Star metric before this (an explicit audit finding). Vanity metrics (signups, DAU) can rise while the actual product promise - owned context sharpening real judgment, compounding over time - goes nowhere. The flywheel is the one number that can only move if a leader is both feeding CTRL real context AND coming back to use it on a real call, which is the product's whole thesis (Corpus §11, the moat) made measurable.
**Trade-off**: A stricter, two-condition metric moves more slowly and is harder to game than a single vanity count, accepted because an easy-to-move number that doesn't track the real loop is worse than no number.
**Outcome**: ✅ Live in prod; view + seed + cron verified (baseline at ship: 10 brain-rich leaders, 1 active decider, 6 weekly-active).

## Decision 62: Decisions Tab Rebuilt as a Radial Force Spider, Replacing the Vertical Claim Ladder (PRs #308-320, 2026-06-30 to 2026-07-01)
**Date**: 2026-07-01
**Decision**: Replace the Decisions tab's vertical claim "ladder" with a diagnostic radial: the decision sits at the centre (a confidence gauge), with the SAME six fixed AI-native forces (Capability / Economics / Risk / Build-vs-buy / Team / Timing) spidering out at fixed positions, each captioned with this decision's specific concern (e.g. Economics -> "Token cost") and colour-coded by health (green holds up / amber shaky / grey only-you-can-call). A leader reads where a decision is strong or weak in one glance; tapping a force opens a one-line verdict then supporting-vs-countering evidence with trust scores. Backend: `decompose` now tags each claim with a fixed `dimension` and emits short per-force labels (`decision_claims.dimension`, `decision_cases.force_labels`, both nullable, no advise-stage change). Frontend: `decisionSpiderModel.ts` (fixed 6-force layout, health rollup, legacy type -> force inference for old untagged claims, centring ported from `brainGraphModel.computeViewBox`) + `DecisionSpider.tsx` (radial SVG canvas ported from `BrainGraph`) + `ForceDrawer.tsx` (mobile sheet / desktop rail). Old, untagged decisions degrade gracefully (claims inferred from type; a force with no claims renders grey, never fabricated). A bundled `demoDecision.ts` renders in the empty state so the tab always shows the right shape.
**Rationale**: A vertical ladder of claims does not answer "where is this decision strong or weak" at a glance; a leader had to read every claim to form that picture themselves. Anchoring on the SAME six AI-native forces every time (not per-decision categories) makes the tab learnable across sessions - a leader who has weighed one decision already knows where to look on the next - while the per-decision caption keeps each force concretely about this call, not generic. Reusing the Brain graph's centring/viewBox math (`computeViewBox`) instead of inventing new layout code kept the radial visually consistent with the rest of the app's canvas language.
**Trade-off**: A graph-shaped diagnostic view is more surface to build and reason about than a flat claim list, and required extending the decomposition schema (a nullable `dimension` field) rather than shipping pure frontend, accepted because "where is this decision weak" answered in one glance is the whole point of a pressure-test surface. Legacy untagged decisions rely on best-effort type inference rather than a real backfill.
**Outcome**: ✅ Shipped and prod-verified; 260 unit tests pass; build + prerender green; typecheck 0 new errors. `docs/CTRL-SYSTEM-SPEC.md` section 1 (the Decisions tab's stable job) is unchanged - this is a same-job redesign of how the tab shows its work.

## Decision 63: Settings Is One Door for Feed/Briefing Tuning, Not Two Duplicate Tables (PR #325, 2026-07-04)
**Date**: 2026-07-04
**Decision**: Fold the standalone `BriefingInterestsTab` (writing `briefing_interests`) into a single reusable `NewsPreferencesPanel`, wrapped by both the Home "Tune feed" drawer and Settings -> Interests. Delete `BriefingInterestsTab`. Alongside: an active-decision control-centre on the track-record "watch" cards (Open / Strengthen / Archive, wired to existing doors via `useDecisionActions`, no new features), a track-record navigation fix (close the settings sheet before routing to `/track-record`, which the global drawer used to cover), new shared main-app design primitives (`Surface` / `Eyebrow` / `SettingRow` / a solid `SheetFooterBar`), and a copy cull of cliché AI-speak across track-record and decision surfaces.
**Rationale**: A leader who tuned their feed from Home's "Tune feed" sheet (writing `news_preferences`) saw no effect in Settings -> Interests (a separate UI over `briefing_interests`), and vice versa - two doors to the same underlying job, silently out of sync. One door means a tune in either place shows everywhere. The design-primitive and copy fixes were bundled because they were found in the same audit pass, not because they are architecturally coupled to the tuning fix.
**Trade-off**: One more shared component (`NewsPreferencesPanel`) to own vs two independently-drifting UIs over two different tables; the people/companies + never-show watchlist still lives in `briefing_interests` (feeding the server briefing lens), now reached only through the unified panel.
**Outcome**: ✅ Live. Typecheck 0 new errors, 337 unit tests pass, production build + prerender green, lint clean on touched files.
