...
- AUTHORITY: on 2026-08-10 the founder approved autonomous completion through production, documentation, merge to `main`, and canonical-domain cutover. No pasted credential was used; existing authenticated GitHub, Supabase, and Vercel sessions were used instead.
...
- DATA: Control Center is a live, fail-closed source adapter inside the existing `live-headlines` pool. It uses a publishable key limited by Control Center RLS to reads, never a cross-project service-role credential. Only high-fit, source-backed rows survive and the original article host retains provenance. A production probe gathered 267 items, retained 211 AI-native items, and included 11 Control Center items.
- DELIVERY: the same ranked pool powers Home and no-login email/audio. Subscriptions are unique by normalized destination, handoffs use a stable idempotency key, result email requires active consent plus an atomic claim, audio is reused on retry, and daily deliveries use a stale-recoverable claim. The headline prewarm and daily email jobs authenticate with one Vault-generated secret mirrored into Edge Function secrets; the retired `app.supabase_service_role_key` setting is no longer a dependency.
...
- VERIFICATION: 856 Vitest tests pass across 52 files; the responsive Playwright suite passes 11/11 First Lens, briefing, and Settings journeys at 1440x900, 390x844, and 320x568. Typecheck, standards, changed-file lint, production build, and 3/3 prerenders pass. Fifteen production Edge Functions are ACTIVE with explicit auth contracts. Live HTTP checks prove public, cron-only, and user-scoped boundaries; the production prewarm returned 10 cards with status 200 and no timeout.
- ACCESS: passwords and API tokens pasted into chat were treated as exposed and were not used. No charge or unrequested external message was created during QA. Those pasted tokens must be revoked or rotated by their owner.
- DEPLOYMENT: the additive database migrations, read-only Control Center bridge, Vault-backed scheduler, and reviewed Edge Functions are deployed and independently read back in production. Vercel deploys the frontend from `main`; `makeyourmindup.ai` is the canonical host and the former CTRL host redirects there.
- ROLLBACK: Supabase function versions before this release were inventoried. Backend rollback is a redeploy from the pre-release `main` revision; frontend rollback is a revert of the release merge followed by the normal Vercel production deployment. Additive tables and columns can remain dormant during rollback.
...
- NON_GOALS: no duplicate products, no generic dashboard, no extra feature pile, no unverified domain cutover, no new data claims unsupported by the current pipeline.
