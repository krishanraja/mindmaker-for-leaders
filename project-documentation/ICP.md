...
**Last reconciled:** 2026-08-09 (drift-check pass: the ICP TODO flagged below is now RESOLVED, not open - `public/.well-known/product.json` ratified the ICP on 2026-08-04, and it is NOT the 50-5,000 employee mid-market frame this file was built on. Also removed the retired Kit program from the pricing anchors.)

> RATIFIED ICP (2026-08-04, `product.json`), read this before anything below it. CTRL's actual beachhead is **AI-native founders and small-team CEOs** - small teams where the founder/CEO is still close to the work, industry-agnostic. The qualifier is not headcount or vertical, it is that they are actively building the AI-native version of their business, already use at least one AI tool daily but keep getting generic output, and have no time and no staff to chase evidence for the calls that matter. Explicitly NOT a fit: enterprise procurement cycles, anyone shopping for consulting/advisory (Mindmaker retired advisory business in July 2026). This directly supersedes the "50-5,000 employees, sweet spot 100-1,000" firmographic frame that makes up most of this file. The file is kept below largely as-written because rewriting the full pain/trigger/success-indicator lists to the founder-CEO frame is real positioning work, not a mechanical drift fix; TODO(founder): either rewrite this file to the ratified founder/small-team-CEO ICP, or confirm the mid-market frame still applies as a secondary segment. Until that happens, treat every company-size/title claim below as SUPERSEDED, and lead outbound with the ratified frame above, not this file's demographics table.
>
> The AI-native fit signals below (paragraph after this banner) are still good instincts and were not contradicted by the ratification.
...
- **Pricing anchors to use in copy**: Free to start (the daily briefing, the Automator, Memory, Voice, and 3 decision weighs a month are all free). $49/month for Edge Pro (the decision tier: unlimited weighs + cross-examination + decision watch + Edge artifacts + the live MCP pull of your skills). Two tiers only - per `product.json`'s guardrails, the one-time Diagnostic/Deep Context/Bundle SKUs are dead and must not be quoted, and the Kit program is RETIRED (2026-08-07, PR #355; `/kit*` now redirects to `/try`).
