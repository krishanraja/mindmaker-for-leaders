...
**Last reconciled:** 2026-07-05 (verified current as a historical record; the two halves that post-date this timeline now also include the 2028 refactor, PR #298, PR #321, and the settings audit - see `CLAUDE.md` for the current state).
...
## Phase 17: Unified onboarding → decisions → engagement loop (PR #298, 2026-06-29)

> Canonical: `docs/CTRL-SYSTEM-SPEC.md` section 8. (Phases 17 onward post-date the kit-redesign and main-app-polish work noted in the intro; this is the next recorded phase.)

### The problem
Onboarding, decisions, and engagement had been built surface by surface, and the seams between them had become the problem. A brand-new leader's first experience depended on a `VITE_COCKPIT_ENABLED` flag (cockpit Home on one side, older Memory dashboards on the other) and a legacy 40-minute voice `OnboardingInterview` auto-offered at first run. The handoff from onboarding into a first weighed decision was loose, so the Decisions area sat unused. And the only re-engagement loop (`decision-watch` raising alerts → `send-daily-briefing`) fired ONLY for a leader who already had decisions AND had opted into daily email - so a leader who set CTRL up but never weighed a decision, or who lapsed, was never brought back. The cold-start trap closed only for the people who least needed it.

### The decision
Adapt the whole experience to the leader's lifecycle state, and match the device mindset. `useCockpit` derives a `userState` (new / dormant / active / power, from real timestamps + a 14-day dormancy window) → a `posture`: `guide` (lead them in with guidance + a kickstart) or `partner` (decisiveness + evidence). Mobile reads as on-the-go (a quick read, one call worth weighing); desktop as deep work (room to think a decision through).

### What shipped
- **Onboarding** became lightweight + inline (`InlineProfileSetup` + `useInlineProfile`): industry + role straight into `user_memory`, interests via the reused `SeedBeatsPrompt`, rendered in the Home feed zone. `Dashboard.tsx` is cockpit-only.
- **First-decision seam**: in the `guide` posture, Home leads with `KickstartCard` - a real, role-tailored starter decision (`src/lib/starterDecisions.ts`) routed into the engine pre-filled. Dormant leaders get a re-kickstart.
- **Re-engagement for everyone**: `send-reactivation-nudge` + a daily cron email NEW and DORMANT leaders into a first/next decision, de-duped on `leader_notification_prefs.reactivation_nudge_sent_at` (30-day re-arm), batch-capped, lifecycle (not the daily-briefing opt-in).
- **No patchwork**: deleted the `VITE_COCKPIT_ENABLED` fork, `legacyNav`, both legacy Memory dashboards, and the entire voice-onboarding tree (`OnboardingInterview`, `DraftCockpit`, `onboarding/steps/*`, `useOnboardingInterview`, the dead `useGuidedCapture` machine, `WelcomeTour`, `ProgressBar`). `EdgeOnboarding` kept.
