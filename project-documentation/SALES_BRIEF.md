...
- **Edge Pro upgrade (live checkout)**: ctrl.themindmaker.ai/upgrade - use this when the ask is "how do I actually subscribe." `/pricing` is a static SEO page with the same $49/month info but no working subscribe button.
- **Cold-lead capture (feature-flagged)**: ctrl.themindmaker.ai/download offers the CTRL starter kit by email to a cold visitor. It sits behind `FF.publicCapture` (default off), so confirm it is live before using it as a standing funnel step in a campaign.
