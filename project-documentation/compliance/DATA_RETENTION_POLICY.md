...
| Kit builds / lesson-kit inputs (`kit_builds.intake`) | Life of account (intended) | Confirmed gap: `kit_builds` is NOT in `delete-account`'s explicit deletion list or its comprehensive user-owned-table sweep as of this review, so kit build/intake rows currently survive account deletion. Open remediation item (add `kit_builds` to the sweep list). |
