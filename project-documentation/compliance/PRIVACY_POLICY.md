...
| Memory Web facts | Thoughts and statements you voice that are extracted into structured facts. Each fact is also written as an AES-256-GCM ciphertext copy as defense-in-depth; the readable copy used for display, search, and AI context is not itself encrypted, and is protected by owner-scoped access control and platform disk encryption instead (see [INFORMATION_SECURITY_POLICY.md](./INFORMATION_SECURITY_POLICY.md) Section 5) | Derived from your inputs |
...
We share personal data only with subprocessors that help us operate the Service, and where required by law. We do not sell your personal data. The full subprocessor register, including purpose, data shared, and location, is maintained in [SUBPROCESSORS.md](./SUBPROCESSORS.md). Subprocessors include Supabase, OpenAI, Anthropic, Google Cloud (Vertex AI / Gemini), ElevenLabs, Stripe, Resend, Perplexity, Tavily, Brave Search, NewsAPI.org, Exa, Artificial Analysis, Jina, Apollo, Google (OAuth and Sheets), PostHog (product analytics), and Vercel.
...
CTRL uses strictly necessary cookies and local/session storage to keep you signed in and to operate the application (for example, authentication tokens managed by Supabase Auth). We do not use third-party advertising cookies.

We use PostHog, a third-party product analytics provider, to capture pageviews, page-leave events, and custom usage events under our legitimate interest in understanding and improving the Service (see [SUBPROCESSORS.md](./SUBPROCESSORS.md)). PostHog is configured with `person_profiles: identified_only`, meaning that when you are signed in, analytics events can be tied to your identified account rather than only aggregated anonymously. We do not use PostHog for advertising, and we do not sell the resulting data. We will surface consent controls for this and any other non-essential analytics where required by law (for example, for EU/UK visitors under applicable e-privacy rules).
...
We apply technical and organizational measures including per-user Row-Level Security, an AES-256-GCM encrypted copy of each Memory fact as defense-in-depth, TLS in transit, provider-managed disk encryption, Stripe webhook signature verification, and rate limiting. See [INFORMATION_SECURITY_POLICY.md](./INFORMATION_SECURITY_POLICY.md) for the full, current-accurate detail (including the honest gap between the encrypted copy and the still-readable copy used for display/search). No system is perfectly secure; if a breach affects you, we will notify you as required by law (see [INCIDENT_RESPONSE_PLAN.md](./INCIDENT_RESPONSE_PLAN.md)).
