...
Last reviewed: 2026-08-20 (verified against production readback; C1.2 and P4 reconciled, third-party minimisation and off the record added)
...
| C1.2 | Disposal of confidential data | A.8.10 | PARTIAL | `delete-account` performs a cascading deletion and now cancels any active Stripe subscription before it runs, so erasure no longer leaves billing active. `cleanup-expired-data` is correct and a daily `retention-cleanup` schedule is committed as `20260820120000_retention_cleanup_cron.sql`, but that migration is **not yet applied to production**: object readback on 2026-08-20 found no such cron job. Until it is applied, per-user retention windows are not enforced automatically. Backups age out on the platform schedule | DATA_RETENTION_POLICY.md; release-state.md |
...
| P4 Use/Retention/Disposal | Retention and deletion | A.8.10 | PARTIAL | Corrected 2026-08-20. This was marked IN PLACE on the strength of the code, but the scheduled cleanup it credited had no schedule and therefore never ran. Configurable retention and cascading deletion are in place; automatic enforcement of the retention window arrives when the cron migration is applied. See C1.2 | DATA_RETENTION_POLICY.md; release-state.md |
...
0. Apply the two committed migrations so retention enforcement and Blind Spot burn actually run, and run the third-party backfill. Until then C1.2, P3 and P4 remain partial.
