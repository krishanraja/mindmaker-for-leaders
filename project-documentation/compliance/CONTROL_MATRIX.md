...
| CC7.2/7.3 | Monitoring and incident detection/response | A.5.24-5.28, A.8.15-8.16 | PARTIAL | Structured JSON logging (CI gate vs console.log); Stripe webhook verification; IR plan documented. `data_audit_log`/`ai_usage_audit` tables exist with owner-scoped RLS (service-role-only writes) but write coverage is narrow (a handful of functions each, not comprehensive); centralized log aggregation/alerting PLANNED/IN PROGRESS | INCIDENT_RESPONSE_PLAN.md; ISP s7 |
...
| CC9.2 | Vendor / subprocessor management | A.5.19-5.23 | PARTIAL | Subprocessor register maintained and reconciled against the actual code (2026-07-19: 12 rows added that were live in code but missing from the register, including a newly-added PostHog analytics integration). Signed DPAs/SCCs + vendor security evidence PLANNED | SUBPROCESSORS.md |
...
| C1.2 | Disposal of confidential data | A.8.10 | PARTIAL | cleanup-expired-data (pg_cron) + delete-account cascade covers most tables; backups age out. `kit_builds` is a confirmed exception not yet swept by delete-account | DATA_RETENTION_POLICY.md |
...
| P4 Use/Retention/Disposal | Retention and deletion | A.8.10 | PARTIAL | Configurable retention + scheduled cleanup + cascading deletion for most tables; `kit_builds` is a confirmed gap not covered by `delete-account` (code-reviewed 2026-07-19) | DATA_RETENTION_POLICY.md |
...
1. Audit logging: `data_audit_log` and `ai_usage_audit` tables and RLS exist, but write coverage is narrow (IN PROGRESS) - needed for monitoring and forensic impact assessment across all data-access and AI-call paths, not just the handful of functions that write to them today.
