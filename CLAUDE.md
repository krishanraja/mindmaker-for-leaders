...
- Skill Builder pipeline (`generate-skill-export`, Edge Pro gated, UNTOUCHED): voice/text transcript → Three Honest Tests triage → OpenAI JSON-mode extraction → quality gate → agentskills.io-compliant ZIP. The default `/context` flow is now the **Automator** (PR #199, merge 24f7d15): `AutomatorFlow` runs brain-mined deliverable suggestions (`useSkillSuggestions` over `user_memory` blockers + decisions, role/sector fallback) → recognition pick-cascade → skill-ready with export, and `automatorModel.composeTranscript` maps the picks into the transcript fed to the unchanged `generate-skill-export`. `SkillCaptureSheet` / `SkillPreviewSheet` are now dead code (unimported).
